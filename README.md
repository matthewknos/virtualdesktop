...
- Backend: a single Vercel serverless function (`api/claude.js`) that proxies all
  Claude API calls. The API key never leaves the server.
...
3. From the prototype JS, call `/api/claude` with `Authorization: Bearer <password>`. Reuse the password-gate pattern from `probation/index.html`.
...
Generate a new key in the Anthropic console, swap `ANTHROPIC_API_KEY` in Vercel, redeploy. The frontend doesn't change.

## Local development

You can preview the static pages just by opening `index.html` in a browser, but `/api/claude` won't work locally without `vercel dev`:
...
Then visit `http://localhost:3000`. Set local env vars in `.env.local` (already gitignored).
...
- The Anthropic key is server-side and never sent to the browser, so even if the password leaks, the key isn't exposed.
