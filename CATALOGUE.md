...
- **Live:** coe-prototypes.vercel.app/sickness-absence (visible on gallery; URL publicly hittable)
- **What it is:** UK-only Workday-resident agent that turns the Sickness Absence Framework from "manager remembers to do it" into "Workday Inbox makes sure it happens" — Loop A drafts an RTW task on every absence-close; Loop B re-tallies the rolling-12-month count + day against Stage 1 thresholds nightly and drafts a stage-review task when crossed; Loop C runs nightly day-of-week + payday + post-weekend clustering across the UK BU and surfaces soft pattern signals when an individual's absence shape diverges from team baseline at p<0.05 (the v1 capability that absorbs roadmap #59 Sickness Absence Pattern Coach into the #22 build). Protected-characteristic absences structurally excluded via a PO-maintained exclusion mechanism. OH referrals and reasonable-adjustment letter drafting are explicit v1 capabilities.
- **Status:** Four-page prototype — landing (`/sickness-absence`), agent design (`/sickness-absence/agent-design`), delivery plan (`/sickness-absence/delivery-plan`), live demo (`/sickness-absence/demo`). Demo is a **pixel-faithful Microsoft Teams desktop replica** with a **live LLM** (`moonshot-v1-32k`) behind the agent persona — not scripted scenarios. Agent output renders as **Adaptive Cards** in mixed mode. Role-aware register: plain English for manager; precise framework terminology for People Operations.
- **Demo principle:** chat, not wireframe. CoE head directive: "identical to Teams." The agent comes to the manager in a conversational surface, not as static UI panels.
